...
def project(depth_image, viz = False):
    ...
    fx, fy = 207, 207  # Focal lengths with 3x down sample
    ...
    bev[:,1] = np.sqrt(bev[:,1]**2 - bev[:,0]**2)
    bev = bev[np.logical_and(bev[:,1]>1 , bev[:,1] < 250)]
...
if __name__ == "__main__":
    ...
    for i in range(500):
        ...
        #depth_image = np.flipud(depth_buf)
        depth_image = np.fliplr(np.flipud(depth_buf))[20:100,2:-2]
        depth_image *= 100
        depth_capped = np.clip(depth_image, None, 300)
        ...
        dir = -1 * track(bev_img)
        if dir > 0.1:
            drive_test.turn(55 + dir * 80)
            print("turn right")
        elif dir < -0.1:
            drive_test.turn(-65 - dir * 80)
            print("turn left")
        else:
            drive_test.turn(0)
        # cv2.imshow('rgb', color_image)
